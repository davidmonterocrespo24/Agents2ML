# TRAINING SCRIPT
# Generated on: 2025-09-08T14:52:43.180481
# Pipeline: pipeline_3ee8ca9e-5834-4f88-a64d-e4a2ec2834dc
# Filename: model_h2o_training.py
# Arguments: --file 3ee8ca9e-5834-4f88-a64d-e4a2ec2834dc.csv --target none --output-dir . --max_runtime_secs 300 --max_mem_size 2G
...
def detect_date_columns(df: pd.DataFrame, thresh=0.75):
    ...
    for col in df.columns:
        if df[col].dtype == object or np.issubdtype(df[col].dtype, np.integer) or np.issubdtype(df[col].dtype, np.floating):
            # FIX_APPLIED: Remove deprecated infer_datetime_format parameter
...
def create_time_features(df: pd.DataFrame, col):
    ...
    df[f"{col}__is_month_end"] = s.dt.is_month_end.astitude(int)
...
def main(args):
    ...
    try:
        ...
        if unsupervised_learning:
            ...
            # FIX_APPLIED: Remove validation frame for unsupervised Isolation Forest training
            # H2O's Isolation Forest requires response column when using validation frame
            leader.train(x=x, training_frame=train)
        ...
        metrics = {}
        try:
            # For unsupervised learning, use training frame for metrics since we don't have validation
            eval_frame = train if unsupervised_learning else (valid if valid is not None else train)
            metrics = extract_metrics(leader, eval_frame, None, is_unsupervised=unsupervised_learning)
        except Exception as e:
