# TRAINING SCRIPT
# Generated on: 2025-09-08T15:01:31.403643
# Pipeline: pipeline_3ee8ca9e-5834-4f88-a64d-e4a2ec2834dc
# Filename: model_h2o_training.py
# Arguments: --file 3ee8ca9e-5834-4f88-a64d-e4a2ec2834dc.csv --output-dir . --max_rows 50000 --max_mem_size 2G
...
def try_read_csv(path, sep=None):
    # Simple CSV reading with fallbacks
    ...
    # Final fallback
    try:
        df = pd.read_csv(path, sep=sep, engine='python', encoding='utf-8', low_memory=False)
        return df, ',', 'utf-8'
    except Exception as e:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--max_rows', type=int, default=50000, help='Maximum rows to use for training')
    parser.add_argument('--ntrees', type=int, default=100, help='Number of trees in Isolation Forest')
    parser.add_argument('--sample_rate', type=float, default=0.8, help='Sample rate for each tree')
    parser.add_argument('--max_depth', type=int, default=8, help='Maximum tree depth')
    parser.add_argument('--max_mem_size', default='4G', help='H2O memory allocation')
