# TRAINING SCRIPT
# Generated on: 2025-09-04T20:42:06.254755
# Pipeline: pipeline_d304ff5d-f2ed-4c30-9955-fa8acbb291b5
# Filename: h2o_training_script.py
# Arguments: --file d304ff5d-f2ed-4c30-9955-fa8acbb291b5_ventas.csv --target monto_total --sep ";" --output-dir ./ --max_models 10 --max_runtime_secs 300 --seed 42 --train-ratio 0.8 --valid-ratio 0.1 --max_rows_for_validation 10000 --max_mem_size 2G --nthreads 2
...
def try_read_csv(path, sep=None, encoding_hints=['utf-8','latin1','cp1252']):
    ...
    for enc in encoding_hints:
        try:
            # FIX_APPLIED: Handle header issue - read without header and assign proper column names
...
def detect_date_columns(df: pd.DataFrame, thresh=0.75):
    ...
    for col in df.columns:
        if df[col].dtype == object or np.issubdtype(df[col].dtype, np.integer) or np.issubdtype(df[col].dtype, np.floating):
            # FIX_APPLIED: Handle Spanish date formats
            try:
...
def main(args):
    ...
    try:
        ...
        # FIX_APPLIED: Assign proper column names instead of using first row as headers
        if len(df.columns) >= 2:
        ...
        # FIX_APPLIED: Handle European decimal format in monto_total
        if 'monto_total' in df.columns:
            # Convert to string first, then replace comma with dot, then convert to float
            df['monto_total'] = df['monto_total'].astype(str).str.replace(',', '.').astype(float)
        
        # FIX_APPLIED: Parse Spanish date format
