# TRAINING SCRIPT
# Generated on: 2025-09-04T15:40:04.557961
# Pipeline: pipeline_e3e0f8c3-d0c6-4620-9df5-fdf5ac58d4e8
# Filename: h2o_training.py
# Arguments: --file e3e0f8c3-d0c6-4620-9df5-fdf5ac58d4e8_ventas.csv --target "monto total" --sep ";" --output-dir ./ --max_models 20 --max_runtime_secs 600 --seed 42 --train-ratio 0.7 --valid-ratio 0.15 --max_rows_for_validation 200000 --max_mem_size 4G --nthreads -1 --stopping_metric AUTO --categorical_unique_ratio_threshold 0.05
...
def parse_spanish_date(date_str):
    # FIX_APPLIED: Handle both "sep" and "Sept." formats in month mapping
    spanish_month_mapping = {
        'ene': 'Jan', 'feb': 'Feb', 'mar': 'Mar', 'abr': 'Apr', 'may': 'May', 'jun': 'Jun',
        'jul': 'Jul', 'ago': 'Aug', 'sep': 'Sep', 'sept': 'Sep',  # Handle both "sep" and "sept"
        'oct': 'Oct', 'nov': 'Nov', 'dic': 'Dec'
    }
    
    # FIX_APPLIED: Handle both abbreviated and full month names with/without periods
    # First, remove any periods from month names and normalize
...
def convert_european_number(num_str):
    # FIX_APPLIED: Convert European decimal format (comma as decimal separator)
    if isinstance(num_str, str):
        # Remove any thousand separators (dots or spaces) and replace comma with dot
        num_str = num_str.replace('.', '').replace(' ', '').replace(',', '.')
    try:
        return float(num_str) if pd.notna(num_str) else np.nan
...
def main(args):
    ...
    try:
        ...
        # FIX_APPLIED: Handle Spanish date format and European numbers
        if len(df.columns) == 2:
            # Rename columns based on typical structure
            df.columns = ['fecha', 'monto total']
            
            # Parse Spanish dates with robust error handling
            df['fecha'] = df['fecha'].apply(parse_spanish_date)
            
            # Convert European decimal format
            df['monto total'] = df['monto total'].apply(convert_european_number)
        
        # Check if date parsing was successful
        if df['fecha'].isna().any():
            print(f"LOG_START:DATE_PARSING_WARNING\n{df['fecha'].isna().sum()} dates failed to parse\nLOG_END:DATE_PARSING_WARNING")
