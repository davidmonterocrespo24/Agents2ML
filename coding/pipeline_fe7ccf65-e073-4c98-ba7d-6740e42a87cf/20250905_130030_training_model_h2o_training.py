# TRAINING SCRIPT
# Generated on: 2025-09-05T13:00:30.210134
# Pipeline: pipeline_fe7ccf65-e073-4c98-ba7d-6740e42a87cf
# Filename: model_h2o_training.py
# Arguments: --file fe7ccf65-e073-4c98-ba7d-6740e42a87cf_ventas.csv --target "monto total" --sep ";" --output-dir ./ --max_models 20 --max_runtime_secs 300 --seed 42
...
def try_read_csv(path, sep=None, encoding_hints=['utf-8','latin1','cp1252']):
    ...
    for enc in encoding_hints:
        try:
            df = pd.read_csv(path, sep=sep, encoding=enc, low_memory=False, decimal=',', header=None)
            return df, sep, enc
        except Exception as e:
            last_exc = e
    # Respaldo: deja que pandas intente autodetectar con motor python
    try:
        df = pd.read_csv(path, sep=None, engine='python', encoding='utf-8', low_memory=False, decimal=',', header=None)
        return df, ',', 'utf-8'
    except Exception as e:
        raise last_exc or e
...
def detect_date_columns(df: pd.DataFrame, thresh=0.75):
    ...
    for col in df.columns:
        if df[col].dtype == object or np.issubdtype(df[col].dtype, np.integer) or np.issubdtype(df[col].dtype, np.floating):
            ...
            parsed = df[col].apply(lambda x: parse_spanish_date(x) if isinstance(x, str) else pd.NaT)
...
def create_time_features(df: pd.DataFrame, col):
    ...
    # Add lag features for time series - FIX_APPLIED: Handle NaN values properly
    target_mean = df['monto total'].mean()
    df[f"{col}__lag1"] = df['monto total'].shift(1).fillna(target_mean)
    df[f"{col}__lag7"] = df['monto total'].shift(7).fillna(target_mean)
    df[f"{col}__rolling_mean7"] = df['monto total'].rolling(window=7, min_periods=1).mean()
...
def main(args):
    ...
    try:
        ...
        # Rename columns since first row contains data, not headers
        df.columns = ['fecha', 'monto total']
        ...
        # ---------- Parse Spanish dates and create features ----------
        date_cols = detect_date_columns(df, thresh=0.5)  # Lower threshold for Spanish dates
        for c in date_cols:
            try:
                df[c] = df[c].apply(parse_spanish_date)
                df = create_time_features(df, c)
                print(f"LOG_START:DATE_FEATURES_CREATED\nColumn:{c}\nFeatures created: year, month, day, dayofweek, etc.\nLOG_END:DATE_FEATURES_CREATED")
            except Exception as e:
                print(f"LOG_START:DATE_PARSE_ISSUE\nColumn:{c}\n{str(e)}\nLOG_END:DATE_PARSE_ISSUE")
        
        # Remove rows with NaN in target or important features
        df = df.dropna(subset=[args.target, 'fecha'])
        ...
        # ---------- Define x, y ----------
        y = args.target
        # FIX_APPLIED: Include all engineered features, don't exclude fecha-based features
        x = [c for c in hf.columns if c != y]  # Include all features except target
        
        # FIX_APPLIED: Validate feature count before training
        if len(x) < 2:
            print("ERROR_START:Insufficient features for training. Need at least 2 features including engineered time features.:ERROR_END")
            return 1
        
        print(f"LOG_START:FEATURES_SELECTED\nTarget: {y}\nFeatures: {x}\nTotal features: {len(x)}\nLOG_END:FEATURES_SELECTED")
        ...
        try:
            mojo_path = leader.download_mojo(path=args.output_dir or "./")
        except Exception:
            mojo_path = None
