# TRAINING SCRIPT
# Generated on: 2025-09-05T12:52:41.793929
...
def try_read_csv(path, sep=None, encoding_hints=['utf-8','latin1','cp1252']):
    ...
    for enc in encoding_hints:
        try:
            df = pd.read_csv(path, sep=sep, encoding=enc, low_memory=False, decimal=',')
            return df, sep, enc
        except Exception as e:
            last_exc = e
    # Respaldo: deja que pandas intente autodetectar con motor python
    try:
        df = pd.read_csv(path, sep=None, engine='python', encoding='utf-8', low_memory=False, decimal=',')
        return df, ',', 'utf-8'
    except Exception as e:
        raise last_exc or e
...
def create_time_features(df: pd.DataFrame, col):
    ...
    df[f"{col}__lag1"] = df['monto total'].shift(1)
    df[f"{col}__lag7"] = df['monto total'].shift(7)
    df[f"{col}__rolling_mean7"] = df['monto total'].rolling(window=7).mean()
...
def main(args):
    ...
    try:
        ...
        # ---------- Define x, y ----------
        y = args.target
        x = [c for c in hf.columns if c != y and not c.startswith('fecha')]  # Exclude original date column
