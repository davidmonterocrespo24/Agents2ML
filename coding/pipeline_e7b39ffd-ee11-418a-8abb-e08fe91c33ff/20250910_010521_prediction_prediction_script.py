# PREDICTION SCRIPT
# Generated on: 2025-09-10T01:05:21.562271
...
def main():
    ...
    try:
        ...
        # FIX_APPLIED: Ensure target column has correct numeric data type (not categorical)
        future_df['total_amount'] = np.nan  # Use NaN with proper numeric type
        
        print(f"LOG_START:Created features for {len(future_df)} future periods:LOG_END")
        print(f"LOG_START:Data types: {future_df.dtypes.to_dict()}:LOG_END")
        
        # Convert to H2OFrame - ensure proper data types
        h2o_future = h2o.H2OFrame(future_df)
        
        # FIX_APPLIED: Explicitly set column types to match training
        # Ensure total_amount is numeric, not categorical
        h2o_future['total_amount'] = h2o_future['total_amount'].asnumeric()
        
        print(f"LOG_START:H2O frame types: {h2o_future.types}:LOG_END")
        
        # Make predictions - handle StackedEnsemble issues
        try:
            predictions = model.predict(h2o_future)
            print("LOG_START:Prediction successful using main model:LOG_END")
        except Exception as e:
            print(f"LOG_START:Main model prediction failed: {str(e)}:LOG_END")
            
            # Try alternative approach: find and use a base model
            try:
                print("LOG_START:Trying alternative prediction approach:LOG_END")
                
                # Look for any non-ensemble model in the same directory
                model_dir = os.path.dirname(args.model_path)
                base_model_found = False
                
                # Try to find GBM, DRF, or GLM models
                for model_file in os.listdir(model_dir):
                    if (model_file.startswith('GBM_') or model_file.startswith('DRF_') or 
                        model_file.startswith('GLM_')) and model_file != os.path.basename(args.model_path):
                        try:
                            base_model_path = os.path.join(model_dir, model_file)
                            base_model = h2o.load_model(base_model_path)
                            predictions = base_model.predict(h2o_future)
                            print(f"LOG_START:Used base model {model_file} for prediction:LOG_END")
                ...
                if not base_model_found:
                    # If no base model found, try creating a simple model as fallback
                    print("LOG_START:No base model found, creating simple fallback model:LOG_END")
                    
                    # Load training data to create a simple model
                    training_data_path = "e7b39ffd-ee11-418a-8abb-e08fe91c33ff_ventas por categoria tecnologia.csv"
                    if os.path.exists(training_data_path):
                        train_df = pd.read_csv(training_data_path, sep=';', header=None)
                        train_df.columns = ['date', 'total_amount']
                        train_df['date'] = pd.to_datetime(train_df['date'], format='%d %b. %Y', errors='coerce')
                        train_df['total_amount'] = train_df['total_amount'].astype(str).str.replace(',', '.').astype(float)
                        ...
                        h2o_train = h2o.H2OFrame(train_df.dropna())
                        
                        # Create a simple GBM model
                        from h2o.estimators import H2OGradientBoostingEstimator
                        simple_model = H2OGradientBoostingEstimator(ntrees=50, max_depth=6, seed=args.seed)
                        simple_model.train(x=['year', 'month', 'day', 'dayofweek', 'is_weekend'], 
                                         y='total_amount', training_frame=h2o_train)
                        predictions = simple_model.predict(h2o_future)
                        print("LOG_START:Used fallback simple model for prediction:LOG_END")
                    else:
                        raise Exception("No training data available for fallback model")
                        
            except Exception as inner_e:
                print(f"ERROR_START:All prediction methods failed: {str(inner_e)}:ERROR_END")
                return 1
