# TRAINING SCRIPT
# Generated on: 2025-09-10T00:35:18.297425
...
def detect_date_columns(df: pd.DataFrame, thresh=0.75):
    ...
    for col in df.columns:
        # Try parsing; allow also object columns
...
def main(args):
    ...
    try:
        ...
        if df.shape[1] == 2:
            df.columns = ['date_raw', args.target]  # target column name as provided
        else:
            # If more than two columns, keep existing names (unlikely for this dataset)
        ...
        # ---------- Date handling ----------
        # Identify date column(s); here we know it's 'date_raw'
        ...
        # Optional: sample for quick validation if dataset is huge
        ...
        # Mark categorical columns as factors (heuristic based on unique ratio)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--target', required=False, default='total amount', help='Name of the target column (y) - optional for unsupervised learning')
    parser.add_argument('--sep', required=False, default=';', help='CSV separator (default: autodetect; set to ; for this dataset)')
    ...
    # Additional optional arguments for advanced AutoML tuning
    parser.add_argument('--sort_metric', type=str, default=None, help='Metric used to sort the leaderboard')
    parser.add_argument('--balance_classes', action='store_true', help='Enable class balancing for classification')
    parser.add_argument('--project_name', type=str, default=None, help='Project name for H2O AutoML')
